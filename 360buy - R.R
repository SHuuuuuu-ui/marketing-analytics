...
numb_clusters_used    <- 3       # heights plot showed a clear jump between merge 2 and 3 — k=3 is the call
distance_used         <- "euclidean"
hclust_method         <- "ward.D"  # ward.D minimises within-cluster variance - best for marketing segments
kmeans_method         <- "Lloyd"
my_cluster_names      <- c("Indifferent Neutrals", "Affluent Early Adopters", "The Digital Loyalists")
...
# mean gives a cleaner story than median here for the snake plot
...
# pie chart - useful for the report to show relative segment sizes
pie_data <- data.frame(
  Segment = factor(my_cluster_names, levels = my_cluster_names),
  Count   = as.numeric(table(cluster_memberships_hclust))
)
ggplot(pie_data, aes(x = "", y = Count, fill = Segment)) +
  ...
  labs(title = "Segment Size Distribution (hclust)", fill = "Segment") +
